...
def enrich_context():
    """
    [FASE E] Recibe una ubicación (texto o coordenadas) y radio, 
    devuelve datos de topografía y clima.
    """
    data = request.get_json(force=True, silent=True) or {}
    location = data.get("location", "")
    latitude = data.get("latitude")
    longitude = data.get("longitude")
    radius = data.get("radius", 200)  # [NUEVO] Radio en metros, default 200
    
    # Validar que al menos se proporcione location o coordenadas
    ...
    try:
        ...
        # Obtener API key de variable de entorno o usar valor por defecto
        api_key = os.environ.get("OPENTOPO_API_KEY", "9c89a797b18ede702687422b4974baa1")
        engine = ContextEngine(api_key_opentopo=api_key)
        
        # [NUEVO] Llamar a enrich_context con todos los parámetros
